...
$\alpha$, $K$, $c$, $\gamma$, $d$, and $g$ are positive constants. 
...
\end{eqnarray}
where $c=\frac{1}{\beta r}$ and
$\gamma=\frac{T_{\mathrm{total}}}{\beta}$.  Looking back at the
...
\begin{eqnarray}
  ...
  \frac{\partial}{\partial t} b(t,\theta) & = &
      \left[ \alpha \cdot p(\theta) b(t,\theta) (K - b(t,\theta)) \right]
      - \left[ \gamma \cdot w(t) \frac{p(\theta) b(t,\theta)}{c+p(\theta)b(t,\theta)} \right]
      + \left[ \mu \frac{\partial^2}{\partial \theta^2} b(t,\theta)\right], \\
  \label{eq:odePDE2}
  \frac{d}{dt} w(t) & = & - \left[ d \cdot w(t) \right]  +
       \left[
          \int^L_{\theta=0} g \cdot w(t) \frac{p(\theta) b(t,\theta) }{c + p(\theta) b(t,\theta)} ~ d\theta
       \right].
\end{eqnarray}
...
increasing function for $0\leq\theta\leq L$, and $p(0)>0$. If
$p(\theta)$ is zero then equation (\ref{eq:initialSystem1}) will
indicate no growth and no predation. We can assume $p(0)$ has a
minimal value, $p(0)=a$, and the relationship has a positive slope. We
...
$\frac{2(1-aL)}{L}$, then the greater the variation is in the observed
trait.
...
\begin{eqnarray*}
  ...
  \frac{\partial}{\partial\theta} b(t,\theta) \bigg|_{\theta=L} & = & 0. \\
\end{eqnarray*}
...
$w^*=\left(\frac{cd}{g-d}+c\right)\left(1-\frac{cd}{\hat{p}(\theta)(g-d)}\right)$.
Note that a requirement for a positive value of $w^*$ is that
$1-\frac{cd}{\hat{p}(\theta)(g-d)}>0$.
