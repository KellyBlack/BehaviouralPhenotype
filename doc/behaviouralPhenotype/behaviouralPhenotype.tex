...
If $\ \left( b\left( t\right) ,w\left( t\right) \right) $ is a solution of
such a system, then by the integrating factor trick

$$\dfrac{d}{dt}\left( e^{\left( -\int_{0}^{t}X(s)ds\right) }b(t)\right) =0$$

Consequently,

$b(t)=e^{\left( -\int_{0}^{t}X(s)ds\right) }b(0)$ and $w(t)=e^{\left( -\int_{0}^{t}Y(s)ds\right) }w(0).$

So for all $t>0$ and $b\left( 0\right) ,w\left( 0\right) >0$ .
\end{proof}

As for boundedness of solutions.
...
We show that $\frac{d}{dt}\left( b(t)+\frac{1}{g}w(t)\right) =\frac{d}{dt}%
\left( R\right) <0$
...
$$\frac{d}{dt}\left( R\right) =\hat{p}\left( \theta \right) b(t)\left(
...
\begin{theorem}
  ...
  ref{eq:scaledODE2}). Assume that the following condition $g>d$ hold
\end{theorem}
...
\begin{figure}[!htp]
\begin{center}
\includegraphics[scale=0.28]{l1.eps}  
\includegraphics[scale=0.28]{l2.eps}
...
\begin{figure}[!htp]
\begin{center}
\includegraphics[scale=0.3]{lim.eps}  
