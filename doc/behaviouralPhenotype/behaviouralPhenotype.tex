...
animals that interact with the species.  Several techniques,  including compartment models, stochastic
models, agent based models, and combinations of these
approaches, are being developed to explore this important aspect of interactions. \textcolor{blue} {I think this needs a list of references} One difficulty, though, with such approaches is the
challenge of developing appropriate analytic tools to examine the
general properties of the resulting models. \textcolor{blue} {I don't get this last sentence, there are plenty of analytic tools available for these types of models.}
...
In this treatment we approximate the system with one  consisting of only two
species, butterflies and wasps.  The chemical associated with mating is meant to give the fertilized eggs a benefit in survival, but paradoxically it is also an attractant to the wasp, which parasitizes the eggs. Finding and parasitizing the butterflies' eggs requires a
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=12cm]{FIGURES/approximation-c-1.1-mu-01-m-01.pdf}
  \caption[Approximation with $c=1.1$, $m=0.01$ and $\mu=0.01$.]{Approximation of
    the system, $m=0.01$, $\mu=0.01$, $c=1.1$, $g=0.6$, and $d=0.1$. }
  \label{fig:approximationMMuC}
\end{figure}

\begin{figure}[htb]
  \centering
  \includegraphics[width=12cm]{FIGURES/approximation-c-1.1-mu-01-m-51.pdf}
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=12cm]{FIGURES/approximation-c-1.1-mu-01-m-150.pdf}
...
\begin{lemma} \label{mm} All the solutions of equations equations
  ...
  $\mathbb R_{+}^{2}$ are uniformly bounded.
\end{lemma}

\begin{proof}
For all $R>0$, consider the triangular region $T_{R}=\left\{ \left(
b,w\right) \in\mathbb R_{+}^{2}:b+\frac{1}{g}w \le R\right\}$
...
In particular, if $R$ is chosen large enough. \textcolor{blue} {I guess I am missing something.  I don't think it is obvious that this is always less than or equal to zero.  If $b(0) <1$, then $b(t)  <1$ for $t>0$ so just take $R > (p+d)/d,$ but if $b(0) > 1$ it is not so obvious that the $R-b(t)$ term stays positive.  I think it is ok, since if $b(0) > 1$, then it's derivative is negative so I guess $R-b(t)$ is bounded by $R-b(0)$.  Anyway, I think this needs to be explained (unless I am missing something more obvious). }
\end{proof}
\begin{theorem}
