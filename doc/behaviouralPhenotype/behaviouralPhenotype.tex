...
\begin{eqnarray}
  ...
  \frac{\partial}{\partial \origT} \origB(\origT,\origTheta) & = &
     \underbrace{ \left[ \origAlpha \cdot \origP{\origTheta} \origB(\origT,\origTheta) (\origK - \origB(\origT,\origTheta)) \right]}_\text{trait dependent logistic growth}
      -  \underbrace{\left[ \origGamma \cdot \origW(\origT) \frac{\origP{\origTheta} \origB(\origT,\origTheta)}{\origC+\origP{\origTheta}\origB(\origT,\origTheta)} \right]}_\text{trait dependent predation by wasps}
      +  \underbrace{\left[ \origMu \frac{\partial^2}{\partial \origTheta^2} \origB(\origT,\origTheta)\right]}_\text{diffusion of trait within population}, \\
  \label{eq:odePDE2}
  \frac{d}{d\origT} \origW(\origT) & = & - \underbrace{\left[ \origD \cdot \origW(\origT) \right]}_\text{natural death}  +
        \underbrace{\left[
          \int^{\origL}_{\origTheta=0} \origG \cdot \origW(\origT) \frac{\origP{\origTheta} \origB(\origT,\origTheta) }{\origC + \origP{\origTheta} \origB(\origT,\origTheta)} ~ d\origTheta
       \right]}_\text{net impact of feeding on butterflies across different trait levels }.
\end{eqnarray}
...
nullclines, there are three fixed points (as long as $g\ne d$), at $(0,0), (1,0)$ and $(b^*,w^*)$ with $b^*=\frac{cd}{p(\theta)(g-d)}$ and
$w^*=\left(\frac{cd}{g-d}+c\right)\left(1-\frac{cd}{p(\theta)(g-d)}\right)$.  This last fixed point is in the positive quadrant only if $g-d>0$ and
...
density at the fixed point is $\frac{u}{p(\theta)}$. Our earlier assumption, equation (\ref{eqn:positivityRequirement}), requiring that the fixed point occurs in the first quadrant,  implies that
\begin{eqnarray}
...
$w^*=\left(\frac{cd}{g-d}+c\right)\left(1-\frac{cd}{p(\theta)(g-d)}\right)$.  The latter fixed point lies in the positive quadrant if $g-d>0$ and $u<p$, where $u=\frac{cd}{g-d}.$
...
parameters the value of $u$ is fully determined. The values of $p=p(\theta)$
