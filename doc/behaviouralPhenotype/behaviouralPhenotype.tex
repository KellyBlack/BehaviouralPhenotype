...
A broad discussion by  \citep{doi:10.1111/j.1461-0248.2010.01536.x} includes a description of variation in behavioural trains and includes multiple examples of the different ways it can impact a population. For example, varying levels of boldness result in different dlevels of exposure. Also, varying levels of social interaction result in different rates of transmissions between individuals. They go on to  propose that the diversity of
...
section, section \ref{section:results}. For the sake of clarity, the notation for the scaled variables will revert to a cleaner notation similar to that originally stated. For example we will express $\scaledB(\scaledT,\scaledTheta)$ as $b(t,\theta)$. For the subsequent sections we will focus on these scaled equations.
...
equations \ref{eq:odePDE1} and \ref{eq:odePDE2}, we explore potential restrictions on the parameters by studying  a closely related system of ordinary differential
equations.  The focus is on the impact of the parameters of the
system, and a more complete analysis is provided in 
...
\ref{fig:uvStabilityRegion}. For the region above the line $u=p$, there is no fixed point in the positive quadrant, but for the region below the line $u = \frac{p-c}{2}$ there is still  a fixed point in the positive quadrant, however, it is not stable.  In the appendix, we will prove that this situation leads to the existence of a \emph{limit cycle}, implying oscillatory behavior of the butterfly and wasp populations.  In addition, it is proven in the appendix that all solutions to the system of ODEs in the first quadrant remain bounded for all future times.  We summarize these results in 
...
\subsection{Numerical Approximation of the solutions of the system of ODEs}
...
\ref{fig:odeButterflyMaxMin}. The system of ODEs given in
...
of $m$ starting at $m=0.1$ and ending at $m=15.0$. The graph on the left, (A), represents the maximum and minimum values of limit cycles for $c=1.1$. The graph on the right, (B), represents the maximum values of limit cycles for $c=2.8$. The impact of increasing the value of $c$ is to shift to a smaller value of $m$ where limit cycles first appear.
...
\begin{figure}[htb]
  ...
  density]{Maximum and minimum values of the butterfly densities after
    a long time period as the parameter $m$ varies.  The value of $c$ is 2.8, the value of $g$ is 0.6, and the value of $d$ is 0.1. }
