...
The variation of animal personalities may play an important role in broader population dynamics\cite{doi:10.1111/j.1461-0248.2010.01536.x}.  The variation
in animal behaviours as well as the variation in the corresponding
genetic basis for animal personalities have multiple role in how they impact interactions\cite{doi:10.1111/j.1461-0248.2010.01536.x,doi:10.1086/687235,mierzejewski_horn_luong_2019,SANTICCHIA20191,doi:10.1098/rspb.2014.1016,FARINE2015609,sibbald2009individual,kurvers2011effect,modlmeier2012diverse,doi:10.1086/687235,doi:10.1098/rspb.2014.1016,doi:10.1037/0735-7036.107.3.250}.  As the recognition for this important phenomena has grown several techniques,  including compartment models, stochastic
models, agent based models, and combinations of these
approaches, are being developed to explore this important aspect of interactions\cite{Keeling65,doi:10.1086/687235,doi:10.1098/rspb.2001.1599,SuperspreadingLloyd,doi:10.1086/687235}.  One difficulty, though, with such approaches is the
challenge of developing appropriate analytic models at the requisite level of granularity consistent with the levels of observed variations. Related to this concern, the method available to analyze the models do not scale well. For example, increasing the levels in a compartment model or an agent based model can make it more difficult to interpret and analyze the resulting system of equations. 

We propose an approach that assumes a continuous distribution
associated with a single behavioural aspect of an animal's behaviour. The temporal dynamics of the change in the distrubtion is modeled as a diffusion process. In
...
While the original system of interactions is complex, involving multiple species, in this paper we consider a simplified system using only two
species, butterflies and wasps.  One chemical associated with mating is meant to give the butterflies a benefit in survival, but paradoxically it is also an attractant to the wasp, which parasitizes the eggs. Finding and parasitizing the butterflies' eggs requires a
...
original model is based on the carrying capacity which presumes that \textcolor{red}{for stability, we need} \textcolor{blue}{I don't really get this, I thought the condition $u/p<1$ gave ensured the critical point is in the first quadrant. Yes, it does imply that the critical $b^* <1$ so the population is below the (rescaled) carrying capacity, but I don't think that is the point.  I would just say something like:  Recall that we earlier assumed $u/p <1$ to get that this critical point exists in the first quadrant. }
\begin{eqnarray}
  \frac{u}{p} & < & 1,
\end{eqnarray}
or 
...
cycle. (Need to check to see if this is consistent with the theory!\textcolor{red}{Related to this, I think the Hopf bifurcation point should be derived explicitly in the section on the ODE, this would make checking these results easier.Given $\theta=1$, I got the Hopf bifurcation point $m_{0}=\frac{2cd}{g-d}-1+c$, which is 2.92 (not quite 2.841 but close)}
